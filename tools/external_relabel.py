...
def main():
    ...
    parser.add_argument(
        "--config-file",
        default="configs/wsup-1000_external_bgnn.yaml",
        metavar="FILE",
    ...
    dic = {}
    for i in l:
        i['img_path'] = f"datasets/vg/50/VG_100k/{i['img_path'].split('/')[-1]}"
    for d in l:
        dic[d['img_path']] = d
    rst_l = [dic[k] for k in file_names]
    os.makedirs(f"{output_dir}/External", exist_ok=True)
    save_path = os.path.join(f"{output_dir}/External", "raw_em_E.pk")

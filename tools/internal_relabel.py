...
def train(cfg, local_rank, distributed, logger):
    ...
    debug_print(logger, "Mapping finished")
    ...
    for iteration, (images, targets, _) in enumerate(train_data_loader):
        ...
        if iteration % 200 == 0:
            logger.info("iters: {}, {}, GPU: {}".format(iteration, len(dic), os.environ['CUDA_VISIBLE_DEVICES']))
        for t, logits in zip(targets, relation_logits):
            cur_data = t.get_field("train_data")
            assert np.all(cur_data["relations"][:, 2] == torch.nonzero(t.get_field("relation_labels"))[:, 1].cpu().numpy())
...
def main():
    ...
    parser.add_argument(
        "--config-file",
        default="configs/wsup-50_internal.yaml",
        metavar="FILE",
    ...
    os.makedirs(f"{output_dir}/Internal", exist_ok=True)
    save_path = os.path.join(f"{output_dir}/Internal", "Internal_em_E.pk")

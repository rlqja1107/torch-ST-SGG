...
class VGDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split, img_dir, roidb_file, dict_file, image_file, transforms=None,
                ...
                filter_duplicate_rels=True, filter_non_overlap=True, flip_aug=False, custom_eval=False, custom_path='',
                 custom_bbox_path='', obj_mapping_file=None):
        """
        ...
        if cfg.MODEL.ROI_RELATION_HEAD.DATA_RESAMPLING and self.split == 'train':
            ...
            assert self.resampling_method == 'bilvl'
    ...
    def __getitem__(self, index):
        ...
        if self.split == "test":
            target = self.get_groundtruth(index, evaluation=True, flip_img=flip_img)
        else:
        ...
        target.add_field("img_path", self.filenames[index])
...
def get_VG_statistics(train_data, img_dir, roidb_file, dict_file, image_file, must_overlap=True, maintain_data=True):
    ...
    for ex_ind in tqdm(range(len(train_data.img_info))):
        ...
        for (o1, o2), gtr in zip(o1o2, gt_relations[:,2]):
            if train_data.obj_mapping is not None:
                o1 = 1; o2 = 1
...
def box_filter(boxes, must_overlap=False):
    ...
    overlaps = bbox_overlaps(boxes.astype(np.float16), boxes.astype(np.float16), to_move=0) > 0

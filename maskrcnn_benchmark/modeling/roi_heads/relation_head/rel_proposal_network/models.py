...
def gumbel_sigmoid(score, temperature):
    sample_prob = RelaxedBernoulli(temperature=temperature, probs=score)
    y = sample_prob.rsample()
    y_soft = y
    y_hard = (y>0.5).to(y.dtype)
    y = (y_hard - y).detach() + y
    return y, score
    # soft_sample = RelaxedBernoulli(temperature=temperature, probs=score).rsample()
    # soft_sample = clamp_probs(soft_sample)
    # hard_sample = QuantizeBernoulli.apply(soft_sample)
    # return hard_sample, score
...
class RelAwareRelFeature(nn.Module):
    def __init__(
        ...
        self.obj_sem_embed = nn.Embedding(len(statistics['obj_classes']), self.embed_dim)
    ...
    def forward(
        ...
        for img_id, (proposal, vis_feats, pair_idx) in enumerate(
            ...
            if self.predictor_type == "hybrid":
                ...
                relness_scores = torch.squeeze(torch.sigmoid(relness_bin_logits))
                pred_rel_matrix[pair_idx[:, 0], pair_idx[:, 1]] = relness_scores.float()
                if len(relness_bin_logits) > 1:
                    relness_logits = torch.cat((relness_logits, relness_bin_logits), dim=1)
                else:
                    relness_logits = torch.cat((relness_logits.view(1,-1), relness_bin_logits.view(1,-1)), dim=1)
...
@registry.RELATION_CONFIDENCE_AWARE_MODULES.register("RelAware_Gumbel")
class RelAware_Gumbel(nn.Module):
    def __init__(
        self,
        input_dim,
    ):
        super(RelAware_Gumbel, self).__init__()
        self.cfg = cfg
        self.num_rel_cls = cfg.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
        ...
        if self.visual_features_on:
            ...
            self.fusion_feat = nn.Sequential(
                nn.LayerNorm(self.hidden_dim * channel),
                nn.ReLU(),
                nn.Linear(self.hidden_dim * channel, self.hidden_dim),
            )                
            # self.fusion_feat = nn.Linear(self.hidden_dim * channel, self.hidden_dim)
    ...
    def forward(
        ...
        num_obj = [len(obj) for obj in entities_proposals]
        num_rel = [len(p) for p in rel_pair_inds]
        relness_matrix = []
        
        if self.visual_features_on: # True
            visual_feat = self.vis_embed(node_feature.detach())
        pre_adj = []
        visual_feat = visual_feat.split(num_obj)
        
        # if rel_pair_global_feat is not None and not isinstance(rel_pair_global_feat, list):
        #     rel_pair_global_feat = rel_pair_global_feat.split(num_rel)
            
        for img_id, (proposal, visual, pair_idx) in enumerate(
            zip(entities_proposals, visual_feat, rel_pair_inds)
        ):
            pair_idx = pair_idx.long()
            device = proposal.bbox.device
            pred_logits = proposal.get_field("predict_logits").detach()

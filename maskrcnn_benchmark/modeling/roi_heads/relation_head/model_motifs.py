...
class FrequencyBias(nn.Module):
    ...
    def __init__(self, cfg, statistics=None, eps=1e-3):
        super(FrequencyBias, self).__init__()
        statistics = torch.load(cfg.OBJ_PRED_INFO_PATH)
        if statistics['pred_dist'] is not None:
    ...
    def forward(self, inputs, initial_state=None, labels=None, boxes_for_nms=None):
        ...
        for i, l_batch in enumerate(batch_lengths):
            ...
            if self.training:
                ...
                is_bg = torch.nonzero(labels_to_embed == 0)
                if is_bg.dim() > 0:
...
class LSTMContext(nn.Module):
    ...
    def __init__(self, config, obj_classes, rel_classes, in_channels):
        ...
        if self.num_obj_classes > 10000:
            print(str("/home/public/Datasets/CV/vg_1800/{}.pt".format(self.num_obj_classes)))
            self.obj_mapping = torch.load(str("/home/public/Datasets/CV/vg_1800/{}.pt".format(self.num_obj_classes)))

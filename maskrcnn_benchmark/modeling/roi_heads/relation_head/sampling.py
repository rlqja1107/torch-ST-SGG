...
class RelationSampling(object):
    ...
    def detect_relsample(self, proposals, targets):
        ...
        for img_id, (proposal, target) in enumerate(zip(proposals, targets)):
            ...
            img_rel_triplets, binary_rel, corrsp_gt_rel_idx = self.motif_rel_fg_bg_sampling(device, tgt_rel_matrix, ious, is_match, rel_possibility, resampling_flag, proposals[0].extra_fields['pred_scores'])
            if target.has_field("relation_non_masked"):
        ...
        return proposals, rel_labels, rel_idx_pairs, rel_sym_binarys, rel_labels_all
    

    def motif_rel_fg_bg_sampling(self, device, tgt_rel_matrix, ious, is_match, rel_possibility, resampling_flag, proposals_quality):
        """
        ...
        return cat((fg_rel_triplets, bg_rel_triplets), dim=0), binary_rel, corrsp_gt_rel_idx
...
class WRelationSampling(object):
    ...
    def gtbox_relsample(self, proposals, targets):
        ...
        for img_id, (proposal, target) in enumerate(zip(proposals, targets)):
            ...
            #     tgt_tail_idxs[0] = 0
            tgt_head_idxs = tgt_head_idxs.long(); tgt_tail_idxs = tgt_tail_idxs.long()
            binary_rel[tgt_head_idxs, tgt_tail_idxs] = 1
            binary_rel[tgt_tail_idxs, tgt_head_idxs] = 1
            rel_sym_binarys.append(binary_rel)
